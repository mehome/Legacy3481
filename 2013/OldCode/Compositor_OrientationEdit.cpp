...
class Compositor
{
	public:
		...
		bool GetIsUsingRotation(const Compositor_Props::Sequence_Packet &seq_pkt) const
		{
			const Compositor_Props &props=m_CompositorProperties.GetCompositorProps();
			typedef Compositor_Props::Shape3D_Renderer_Props ShapeProps;
			...
			const bool AdjustRotation=SmartDashboard::GetBoolean("Adjust_Rotation");
			bool UsingRotation=false;
			const bool IsSquare=(seq_pkt.type==Compositor_Props::eShape3D)&&(shape_props.draw_shape==ShapeProps::e_Square);
			UsingRotation=IsSquare && AdjustRotation;
		...
		void UpdateSequence(size_t NewSequenceIndex,bool forceUpdate=false)
		{
			if ((m_SequenceIndex!=NewSequenceIndex)||forceUpdate)
			{
				...
				Compositor_Props::Shape3D_Renderer_Props &shape_props_leaving_sequence_rw=props_rw.shapes_3D_reticle[Sequence[m_SequenceIndex].specific_data.SquareReticle_SelIndex];
				ShapeProps::type_specifics::Shapes2D_Orientation_Props &orientation_leaving_sequence_rw=shape_props_leaving_sequence_rw.specific_data.Shapes2D_Orientation;

				const bool UsingRotation=GetIsUsingRotation(Sequence[m_SequenceIndex]);
				const Compositor_Props &props=m_CompositorProperties.GetCompositorProps();
				...
				else if (Sequence[m_SequenceIndex].type==Compositor_Props::eShape3D)
				{
					orientation_leaving_sequence_rw.Yaw=m_Xpos;
					orientation_leaving_sequence_rw.Pitch=m_Ypos;
				}
				...
				case Compositor_Props::eShape3D:
					if (!UsingRotation)
						...
						orientation_leaving_sequence_rw.Roll=m_Zpos;
					break;
				}
				...
				const Compositor_Props::Shape3D_Renderer_Props &shape_props=props_rw.shapes_3D_reticle[seq_pkt.specific_data.SquareReticle_SelIndex];
				const ShapeProps::type_specifics::Shapes2D_Orientation_Props &orientation=shape_props.specific_data.Shapes2D_Orientation;
				...
				{
					m_Xpos=orientation.Yaw;
					m_Ypos=orientation.Pitch;
				}
			...
			m_IsEditable(false),m_PreviousIsEditable(false),m_RecurseIntoComposite(false),m_Flash(false),m_ToggleLinePlot(true)
		{
			...
			SmartDashboard::PutBoolean("Adjust_Rotation",false);
			#ifdef __Edit_FOV__
			...
			case Compositor_Props::eShape3D:
				{
					...
					{
						const bool UsingRotation=GetIsUsingRotation(seq_pkt);
						...
						{
							Xpos=seq_pkt.PositionX+XOffset;
							Ypos=seq_pkt.PositionY+YOffset;
							Zpos=seq_pkt.specific_data.ShapeReticle_props.PositionZ;
							ShapeProps::type_specifics::Shapes2D_Orientation_Props &orientation_rw=shape_props_rw.specific_data.Shapes2D_Orientation;
							orientation_rw.Yaw=m_Xpos;
							orientation_rw.Pitch=m_Ypos;
							orientation_rw.Roll=m_Zpos;
						}
						...
						if (shape_props.RemoteVariableName.c_str()[0]!=0)
						{
							...
							else
							{
								if (shape_props.draw_shape==ShapeProps::e_Square)
								{
									sTest=shape_props.RemoteVariableName;
									sTest+="_rot_x";
									SmartDashboard::PutNumber(sTest,RAD_2_DEG(m_Xpos));
									sTest=shape_props.RemoteVariableName;
									sTest+="_rot_y";
									SmartDashboard::PutNumber(sTest,RAD_2_DEG(m_Ypos));
									sTest=shape_props.RemoteVariableName;
									sTest+="_rot_z";
									SmartDashboard::PutNumber(sTest,RAD_2_DEG(m_Zpos));
								}
							}
						}
					}
