...
//      of the seek_pos/seek_rel variables
        ...
        if (ret < 0) 
		{
			...
			{
				using namespace FrameWork;
				DebugOutput("***TimeOut detected; reset() issued\n");
				Reset();
			}
            continue;
        }
...
struct FF_Play_Reader : public FF_Play_Reader_Internal
{
	public:
		FF_Play_Reader();
		void SetFailSafeReset_Interface(FailSafeReset_Interface *ResetMethod) {m_ResetInterface=ResetMethod;}
		bool StartStreaming();
		void StopStreaming();
	protected:
		virtual void CreateVideoStream();
	...
	private:
		...
		bool m_IsStreaming;
		FailSafeReset_Interface * m_ResetInterface;
};

FF_Play_Reader::FF_Play_Reader() : m_pReaderThread(NULL),m_pVideoThread(NULL),m_IsStreaming(false),m_ResetInterface(NULL)
{
...
void FF_Play_Reader::Reset() 
{
	if (m_ResetInterface)
		m_ResetInterface->Reset();
}
...
bool FrameGrabber_FFMpeg::StartStreaming()
{
	...
	reader->SetFailSafeReset_Interface(this);
...
void FrameGrabber_FFMpeg::Reset()
{
	...
	using namespace FrameWork;
	cpp::threadcall_ex( do_not_wait, m_thread, this, &FrameGrabber_FFMpeg::Reset_Thread);
}
