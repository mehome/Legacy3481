...
class SetUp_Manager
{
	protected:
		...
		FRC_2014_Robot_Properties m_RobotProps;
		FRC_2014_Robot_Control m_Control; // robot drive system
		FRC_2014_Robot *m_pRobot; //This is a scoped pointer with late binding
		UI_Controller *m_pUI;
	...
	public:
			...
			m_JoyBinder(m_Joystick),m_Control(UseSafety),m_pRobot(NULL),m_pUI(NULL)
		{
			...
			m_pRobot = new FRC_2014_Robot("FRC2014_Robot",&m_Control,UseEncoders);
			{
				...
				script.LoadScript("/FRC2014Robot.lua",true);
		...
		FRC_2014_Robot *GetRobot() const {return m_pRobot;}
...
public:
	...
	void OperatorControl(void)
	{
		...
		else
		{
			...
			FILE *test=fopen("/FRC2014Robot.lua","r");
			if (test)
			{
				char buffer[80];
