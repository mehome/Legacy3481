...
{	// Start the server
	...
	if ( config::debug_receive_creation )
		FC3::debug::debug_output( FC3::config::debug_category, L"raw server created, %s", name );	
...
const FC3::raw::message* pull::operator() ( const DWORD time_out )
{	// Get a message
	DWORD block_id, addr;
	if ( !__super::pull( block_id, addr, time_out ) ) return false;

	// Create a message
	FC3::raw::message	*p_new_msg = new FC3::raw::message( block_id, addr );
