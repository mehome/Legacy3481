...
void JoyStick_Binder::SetControlledEventMap(FrameWork::EventMap* em)
{
...
void JoyStick_Binder::UpdateJoyStick(double dTick_s)
{
	...
	for (size_t JoyNum=0;JoyNum<m_Joystick.GetNoJoysticksFound();JoyNum++)
	{
		...
		const IJoystick::JoystickInfo &joyinfo2=m_Joystick.GetJoyInfo(JoyNum);
		if (m_Joystick.read_joystick(JoyNum,joyinfo))
		{
			...
			IJoystick::JoyState &floodcontrol=m_FloodControl[JoyNum];
