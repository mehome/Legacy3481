...
/*
//Whatever! I don't want to post this, using the VS compiler does just as well if not better  [11/18/2008 James]
#ifndef __ICL
...
public:
	...
	{
		//James was here... we need to use unaligned load and stores!
		_mm_storeu_ps((float *)&_L1,_mm_loadu_ps((float *)&m._L1));
		_mm_storeu_ps((float *)&_L2,_mm_loadu_ps((float *)&m._L2));
		_mm_storeu_ps((float *)&_L3,_mm_loadu_ps((float *)&m._L3));
		_mm_storeu_ps((float *)&_L4,_mm_loadu_ps((float *)&m._L4));
    ...
    SPMatrix& operator = (const SPMatrix& a) {
		...
		#else
		//James was here... we need to use unaligned load and stores!
		_mm_storeu_ps((float *)&_L1,_mm_loadu_ps((float *)&a._L1));
		_mm_storeu_ps((float *)&_L2,_mm_loadu_ps((float *)&a._L2));
		_mm_storeu_ps((float *)&_L3,_mm_loadu_ps((float *)&a._L3));
		_mm_storeu_ps((float *)&_L4,_mm_loadu_ps((float *)&a._L4));
		#endif
        return *this;
    }
...
extern const _MM_ALIGN16 __int32 __MASKSIGNs_[4];
