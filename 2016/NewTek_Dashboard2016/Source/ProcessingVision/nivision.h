...
typedef struct CalibrationCorrectionLearnSetupInfo_struct CalibrationCorrectionLearnSetupInfo;
typedef struct CalibrationModelSetup_struct CalibrationModelSetup;
...
typedef struct FindCircularEdgeReport_struct {
    ...
    double     avgStrength;      //Average strength of the edge found.
    double     avgSNR;           //Average SNR (Signal to Noise Ratio) for the edge found.
    int        circleFound;      //Specifies if the circlular edge was found.
} FindCircularEdgeReport;

typedef struct WindowSize_struct {
    int x;        //Window length on X direction.
    int y;        //Window length on Y direction.
...
typedef struct CooccurrenceOptions_struct {
    ...
    DisplacementVector displacement; //Displacement between pixels to accumulate the matrix.
} CooccurrenceOptions;
