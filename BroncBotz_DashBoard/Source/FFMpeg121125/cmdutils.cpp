...
void init_opts(void)
{
    ...
    if(CONFIG_SWSCALE)
        sws_opts = sws_getContext(16, 16,(AVPixelFormat) 0, 16, 16,(AVPixelFormat) 0, SWS_BICUBIC, NULL, NULL, NULL);
...
void show_help_children(const AVClass *av_class, int flags)
{
    ...
    while (child = av_opt_child_class_next(av_class, child))
...
static void expand_filename_template(AVBPrint *bp, const char *fileTemplate,
                                     struct tm *tm)
{
    int c;

    while ((c = *(fileTemplate++))) {
        if (c == '%') {
            if (!(c = *(fileTemplate++)))
                break;
            switch (c) {
...
static int init_report(const char *env)
{
    ...
    while (env && *env) {
        if ((ret = av_opt_get_key_value(&env, "=", ":", 0, &key, &val)) < 0) {
            if (count)
                av_log(NULL, AV_LOG_ERROR, "Failed to parse FFREPORT environment variable: %d\n", ret);
            break;
        }
        if (*env)
            env++;
...
static int alloc_buffer(FrameBuffer **pool, AVCodecContext *s, FrameBuffer **pbuf)
{
    ...
    buf = (FrameBuffer *)av_mallocz(sizeof(*buf));
    if (!buf)
        return AVERROR(ENOMEM);
