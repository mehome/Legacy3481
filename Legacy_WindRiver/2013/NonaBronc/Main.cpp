...
class SetUp_Manager
{
	protected:
		...
		Nona_Robot_Properties m_RobotProps;
		Tank_Nona_Control m_Control; // robot drive system
		Nona_Robot *m_pRobot; //This is a scoped pointer with late binding
		UI_Controller *m_pUI;
	...
	public:
			...
			m_JoyBinder(m_Joystick),m_Control(UseSafety),m_pRobot(NULL),m_pUI(NULL)
		{
			...
			m_pRobot = new Nona_Robot("FRC2013_Robot",&m_Control,UseEncoders);
			{
		...
		Nona_Robot *GetRobot() const {return m_pRobot;}
...
public:
	...
	void Autonomous(void)
	{
		...
		Nona_Robot *Robot=m_Manager.GetRobot();  //we can always cast down
