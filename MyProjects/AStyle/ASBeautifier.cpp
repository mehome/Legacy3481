...
ASBeautifier::ASBeautifier()
{
	...
	setBracketIndentBottomOnly(false);
...
{
	if (state)
		setBracketIndentBottomOnly(false);   //BrackIndent... and the bottom only are mutually exclusive
    bracketIndent = state;
}

void ASBeautifier::setBracketIndentBottomOnly(bool state)
{
	if (state)
		setBracketIndent(false);		//BrackIndent... and the bottom only are mutually exclusive
    bracketIndentBottomOnly = state;
}
...
*/
    ...
    else if (bracketIndentBottomOnly && outBuffer.length()>0 && shouldIndentBrackettedLine) 
	{
        if (outBuffer[0]=='}')
            tabCount++;
	}
