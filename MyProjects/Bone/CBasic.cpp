...
int error(char *msg,char msgtype) {

	if (msgtype==0) {
		MessageBeep(MB_ICONSTOP);
		MessageBox(screen,msg,"Error!",MB_OK|MB_ICONSTOP|MB_SYSTEMMODAL);
		cleanup();
		throw msg;
		}
	...
	if (msgtype==1) {
		...
		MessageBox(screen,msg,"Warning",MB_OK|MB_ICONEXCLAMATION|MB_SYSTEMMODAL);
		}

	if (msgtype==2) {
		MessageBeep(MB_ICONEXCLAMATION);
		return(MessageBox(screen,msg,"Warning",MB_RETRYCANCEL|MB_ICONEXCLAMATION|MB_SYSTEMMODAL));
		}
...
APTR newmemnode(
	...
	if (object->root_ind+size>object->heap) {
		mem=mynew(&object->pmem,object->heap);
...
struct savevars *createsave(
	...
	mem->heap=heap;
	mem->root_mem=(char *)mynew(pmem,sizeof(heap));
	mem->root_ind=0;
	return(mem);
	}
