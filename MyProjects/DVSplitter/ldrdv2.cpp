...
// 20lin: even: +=2 odd: +=3
/*@/// inline short decode_20lin(unsigned char* pc_pcm,bool is_even) */
inline short decode_20lin(unsigned char* pc_pcm,bool is_even)
{
  short result;
  if (is_even)  // even
  {
    result = (pc_pcm[0]<<8)+(pc_pcm[1]);  // pc_pcm[2] msn is dropped  // !!! could use bswap instead
  }
  else  // odd
  {
    result = (pc_pcm[1]<<8)+(pc_pcm[2]);  // pc_pcm[0] lsn is dropped  // !!! correct order of bytes?  // !!! could use bswap instead
  }
  return result;
}
/*@\\\0000000A01*/
// 16lin:
/*@/// inline short decode_16lin(unsigned char* pc_pcm) */
inline short decode_16lin(unsigned char* pc_pcm)
{
  short result;
  result = (pc_pcm[0]<<8)+(pc_pcm[1]);  // !!! could use bswap instead
  return result;
}
/*@\\\0000000427*/
// 12lin: even: +=1 odd: +=2
/*@/// inline short decode_12nonlin(unsigned char* pc_pcm,bool is_even) */
inline short decode_12nonlin(unsigned char* pc_pcm,bool is_even)
{
  short result;
  ...
  //static short a_12_add[] = { 0x0000,0x0000,0xff00,0xfe00,0xfd00,0xfc00,0xfb00,0xfa00, 0x0601,0x0501,0x0401,0x1301,0x3201,0x7101,0xf000,0xf000 };
...
DLLExportC int isProject(FILE* f)
{
  ...
  if (ul_chars_read == 8*sizeof(char))
  {
    ...
    if ((ac_buf[3]&0x3f) != 0x3f) return 0;  // reserved, this is testable (0xbf "hxxx" | 0x3f "hxxx")
    if (*(long*)&ac_buf[4] != 0x78787868) return 0;  // HDR data type info HDR[3..7] says it's DV (consumer) video
  }

  DosSetFilePtr(f,0,FILE_BEGIN,&ul_chars_read);  // !!! 64bit
  return 1;                                            /* yes == DV Return false */
}
/*@\\\0000000C01*/
...
ctor  constructor (t())
cuc   currently under construction
dtor  destructor (~t())
fyi   for your information
imho  in my humble opinion
kiss  keep it simple, stupid
mop   master of puppets (to be MOPped by sth: fig. to be controlled by sth)
...
tbo   to become obsolete
tch   this can't happen
toc   table of contents
w/    with
w/o   without
wysiwyg what you see is what you get
wygiwyw what you get is what you wrote (not what you meant)

tla   three letter acronym
sfla  stupid four letter acronym
yafla yet another five letter acronym
aaaaa american association against acronym abuse
tacatcgagttgc that's as cool as that can get, all's going to the ground, crashing (now that they are decoding genes...)
*/
/*@\\\0000002101*/
/*@/// type/var names: (reverse) hungarian notation */
/* type/var names: (reverse) hungarian notation
* general
  (reverse) hungarian notation - why?

  the use of hungarian notation is not necessary, the compiler does the type-
    checking itself. so why the hassle?
  the answer, in short, is that experience holds that visually identifiable
    variable types help programmers program more easily. for one, you don't
    have to look up a variable's type all the times, especially if you are
    new to a piece of code. another point is, that when you use prefixes and
    postfixes to separate semantically bound variables, you can use the same
    "main" name to visually link them too (eg n_dump is the filename's string,
    f_dump is the related QFile object, eg as_device_name[] is an array of
    strings for device names, device_num the accompanying count of elements,
    device_nr the number of the current element - you get the drift).

* prefixes (functions: prefix for result type, postfix for param type)
_   parameter (optional)
a   array
b   (binary) tree
c   char/QChar (optional, needed in classes/structs/unions)
d   double (optional, needed in classes/structs/unions)
dic dictionary (dic_ QDict string key, idic_ QIntDict int key)
e   exception
env environment
f   file
fl  float (optional, needed in classes/structs/unions)
fp  fixed point integer (optional, needed in classes/structs/unions)
g   global variables
h   handle (system handles, ptr to ptr)
i   integer (optional, needed in classes/structs/unions)
k   timer (klock)
l   list, eg list of string: ls_
lo  long (optional, needed in classes/structs/unions),
m   member (variables inside the class' structure)
n   filename (string)
p   pointer
q   enum
r   struct/union
...
sh  short
t   type
u   unsigned (optional, needed in classes/structs/unions)
w   wide char (swz_, nwz_, not needed with QString/QChar)
x   external (base types like [unsigned] char/short/long/__int64, embedded in a struct, to make external ids incompatible to internal ids)
y   boolean (sometimes)
z   asciiz (eg: sz_ -> char[] string terminated by '\0', nz_)
is/has/does  boolean (eg is_running, has_vcr_ctrl)

* postfixes
iter  iterator
max   maximum number of elements
nr    number of current element
...
* variable names
result
  i propose to generally use "result" as the name for any variable used to
    "return result".
  sometimes, you'll need a variable to hold the result of some calculations
    (eg w/ strings), and at some time, a keyword might be introduced to help
    compilers to further optimize returning values to callers. if such a
    keyword is ever introduced, a common name for such a variable would help
    adopting this feature. visually, the hungarian notation effects are lost,
    but all the same there's a common name then.

* parameter sequence
dest,source
  if possible, it would be in (C's) tradition to start parameters with the
    destination variable (ptr or ref), then follow with the source parameters.
    this way, a function call can be almost read "infix".
    eg: mem_copy_8(dst,src,length);
  if there's a source variable that would be complex expressions most of the
    times, or if chaining of functions on one of the source variables is
    possible, try to make that parameter last. this way, a function call chain
    can be read from inside out, and behind the inmost call there's only closing
    brackets.
    eg: n_dump = s_replace(' ' ,'_',
                 s_replace('\\','/',n_dump));

* if statement
  if possible, try to organize the condition expression so that the "then"
    statement is the shortest or the simplest - this makes the if statement
    more simple to grasp, especially if it is only one statement.
  else-if chains: i like start those with "if (0);" - that makes all branches
    "else if ()" branches, so they are easy to move up or down the else-if
    chain, and all branches can be "#ifdef"-ed out.
*/
/*@\\\0000005B01*/
/*@\\\0000000301*/
