...
#pragma comment(lib, "beethoven")
#pragma comment(lib, "newtekrtme")
#pragma comment(lib, "proof")
...
void consume_mediagram (void *userdata,void *media_buffer,const union MediaGram *mg) {
	...
	if ((field_num==0)||(field_num==1)) {
		...
		memcpy(((char *)(capture->rtvcache[toastercapobj->frameindex]))+(field_num*size_of_data),media_buffer,size_of_data);
		if (field_num==1) {
...
/**/
	if (rtc=rtc_InitializeRTME()) { //We'll let storyboard run w/o toaster
		rtmeoutput=rtme_output_Create("Apps.Exodus.VTNT_DV_kit",0,0,false);
...
/**/
	if (!rtmeoutput) {
		NoToaster=new ULONG[172800];
		videobuf=(ULONG *)NoToaster;
		videooff=1;
		}
	}
...
/**/
	for (t=start;t<start+RTVCACHEHALF;t++) {
		...
		WriteRTVFile(toastercapobj->videofilename,capture->rtvcache[t]);
		}
/**/
