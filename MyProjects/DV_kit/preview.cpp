...
void previewclass::createwindow(HWND w_ptr) {
	prevwindow=CreateWindowEx(0,"OBJGRAYWIN","Preview",
		WS_POPUP|WS_OVERLAPPEDWINDOW,250,300,235,200,w_ptr,
		NULL,hInst,NULL);
	SetWindowLong(prevwindow,GWL_USERDATA,(long)this);
	if (!lpDD4) overlaysupport=setuppreview();
	configuresize();
	if (overlaysupport) updatepreview();
	else EnableMenuItem(m_ptr,IDM_PREVWINDOW,MF_GRAYED);
	hideoverlay();
	}

void previewclass::resize() {
	//size100(screen,prevwindow,0,70,27,100);
	configuresize();
	}
...
BOOL previewclass::initFail(char *reason)  {
	printerror("Warning: No Overlay Support Detected Reason: ");
	printc(reason);
	printc("Preview window is not available");
	return (FALSE);
	} // End initFail
