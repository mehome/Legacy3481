...
void consoleclass::createwindow(HWND w_ptr) {
	consolewindow=CreateWindowEx(0,"EDIT","",WS_VISIBLE|WS_DLGFRAME|
		WS_CHILD|ES_AUTOVSCROLL|WS_VSCROLL|
		ES_MULTILINE|ES_WANTRETURN|ES_LEFT,235,270,300,300,w_ptr,
		(HMENU)IDC_CONSOLE,hInst,NULL);
	}

void consoleclass::resize(LONG y) {
	size100noty(screen,consolewindow,27,y,100,100);
	}
...
void consoleclass::print(char *string) {
	...
	if (length>2000) {
		//error(1,"Overflow need to reset buffer");
		clearconsole();
		}
