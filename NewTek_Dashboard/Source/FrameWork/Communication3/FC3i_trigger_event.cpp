...
{	// Create the trigger name
	const size_t len_pre  = ::wcslen( FC3::config::name_trigger );
	const size_t len_name = 8;
	wchar_t *p_tgr_name   = (wchar_t*)_alloca( ( len_pre + len_name + 1 ) * sizeof( wchar_t ) );
	::swprintf( p_tgr_name, L"%s%08X", FC3::config::name_trigger, trigger_id );
	...
	{	debug_output( FC3::config::debug_category, L"New trigger (%d) : %s\n", g_debug_no_objects, p_tgr_name );
...
{	// Debugging
	...
	{	::_InterlockedDecrement( (LONG*)&g_debug_no_objects );
		debug_output( FC3::config::debug_category, L"Del trigger (%d)\n", g_debug_no_objects );
	}
}
