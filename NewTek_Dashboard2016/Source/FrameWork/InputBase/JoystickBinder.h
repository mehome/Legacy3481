...
public:
	KeyboardMouse_CB() {}
	bool AddKeyBinding(FrameWork::Key key,const std::string eventName, bool useOnOff, bool ForceBindThisKey=false) {return false;}
	void RemoveKeyBinding(FrameWork::Key key, std::string eventName, bool useOnOff) {}

	/// This version of the function is just for easy porting from the old KB technique
	void AddKeyBindingR(bool useOnOff, std::string eventName, FrameWork::Key key)
...
public:
	...
	FrameWork::IJoystick &GetJoystick() const;
	...
	JoyStick_Binder(FrameWork::IJoystick &joystick);
	~JoyStick_Binder();
	...
	//stresses presented 
	//  [2/5/2010 JamesK]
	void SetControlledEventMap(FrameWork::EventMap* em);
...
private:
	...
	FrameWork::IJoystick &m_Joystick;
	FrameWork::EventMap *m_controlledEventMap;
	std::vector<FrameWork::IJoystick::JoyState> m_FloodControl;
