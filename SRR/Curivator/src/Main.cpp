...
class SetUp_Manager
{
	protected:
		...
		Curivator_Robot_Properties m_RobotProps;
		Curivator_Robot_Control m_Control; // robot drive system
		Curivator_Robot *m_pRobot; //This is a scoped pointer with late binding
		UI_Controller *m_pUI;
	...
	public:
		...
		{
			SmartDashboard::init();
			m_pRobot = new Curivator_Robot("Curivator_Robot",&m_Control,UseEncoders);
			{
				Framework::Scripting::Script script;
				script.LoadScript("/CurivatorRobot.lua",true);
...
public:
	...
	void OperatorControl(void)
	{
		...
		else
		{
			...
			FILE *test=fopen("/CurivatorRobot.lua","r");
			if (test)
			{
				char buffer[80];
