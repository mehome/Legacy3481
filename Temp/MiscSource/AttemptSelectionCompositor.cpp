...
struct Compositor_Props
{
	...
	struct Sequence_Packet
	{
		...
		union type_specifics
		{
			...
			Sequence_Container *Composite;
		} specific_data;
	};

	Sequence_Container Sequence;

	//This takes care of creating the deletion list implicitly
	Sequence_Container *CreateCompositeList() 
	{
		Sequence_Container *NewList=new Sequence_Container;
	...
	std::vector<Sequence_Container *> Composite_List;  //keep track of all composite lists created for deletion
};
...
Compositor_Properties::Compositor_Properties() : m_CompositorControls(&s_ControlsEvents)
{
	...
	props.Sequence.list.push_back(seq_pkt);
	props.Sequence.CurrentSelection=0;
	...
	do 
	{
		...
		if (!err)
		{
			...
			case Compositor_Props::eComposite:
				...
				if (!err)
				{
					Compositor_Props::Sequence_Container *NewList=props.CreateCompositeList();
					seq_pkt.specific_data.Composite=NewList;
					//recursively call this with a new composite list to be used as the sequence
					LoadSequenceProps(script,NewList->list,props,true);
					script.Pop();
				}
				break;
			}
...
static void LoadSequence_PersistentData(Scripting::Script& script,Compositor_Props &props)
{
	...
	do 
	{
		...
		if (!err)
		{
			Compositor_Props::Sequence_Packet &seq_pkt=props.Sequence.list[index-1]; //ordinal translation
...
void Compositor_Properties::LoadFromScript(Scripting::Script& script)
{
	...
	if (!err)
	{
		...
		{
			//clear defaults
			props.Sequence.list.clear();  //for now it is assumed the default will not allocate a composite
			LoadSequenceProps(script,props.Sequence.list,props);
...
class Compositor
{
	public:
		...
		void UpdateSequence(Compositor_Props::Sequence_Container &sequence,size_t NewSequenceIndex,bool forceUpdate=false)
		{
			if ((sequence.CurrentSelection!=NewSequenceIndex)||forceUpdate)
			{
				Compositor_Props &props_rw=m_CompositorProperties.GetCompositorProps_rw();
				//Save current position to this sequence packet
				sequence.list[sequence.CurrentSelection].PositionX=m_Xpos;
				sequence.list[sequence.CurrentSelection].PositionY=m_Ypos;

				//issue the update
				sequence.CurrentSelection=NewSequenceIndex;

				const Compositor_Props &props=m_CompositorProperties.GetCompositorProps();
				if (sequence.CurrentSelection==-1)
					sequence.CurrentSelection=sequence.list.size()-1;
				else if (sequence.CurrentSelection>=sequence.list.size())
					sequence.CurrentSelection=0;

				SmartDashboard::PutNumber("Sequence",(double)(sequence.CurrentSelection+1));
				//Modify position to last saved
				m_Xpos=sequence.list[sequence.CurrentSelection].PositionX;
				m_Ypos=sequence.list[sequence.CurrentSelection].PositionY;
			}
		}
		...
		{
			Compositor_Props &props_rw=m_CompositorProperties.GetCompositorProps_rw();
			UpdateSequence(props_rw.Sequence,props_rw.Sequence.CurrentSelection+1);
		}
		...
		Compositor(const char *IPAddress,Dashboard_Framework_Interface *DashboardHelper) : m_Bypass(IPAddress,DashboardHelper),m_JoyBinder(FrameWork::GetDirectInputJoystick()),
			m_BlinkCounter(0),m_Xpos(0.0),m_Ypos(0.0),m_IsEditable(false)
		{
		...
		void SaveData()
		{
			...
			out << "{" << endl;
			const Compositor_Props::Sequence_List &sequence= m_CompositorProperties.GetCompositorProps().Sequence.list;
		...
		~Compositor()
		{
			size_t SequenceIndex=m_CompositorProperties.GetCompositorProps().Sequence.CurrentSelection;
			//ensure the current position is set on the properties
			{
				Compositor_Props::Sequence_List &sequence_rw= m_CompositorProperties.GetCompositorProps_rw().Sequence.list;
				Compositor_Props::Sequence_Packet &seq_pkt_rw= sequence_rw[SequenceIndex];
			...
			SmartDashboard::PutNumber("Sequence",(double)(SequenceIndex+1));
		...
		virtual void Initialize(EventMap& em, const Compositor_Properties *props=NULL)
		{
			...
			if (props)
			{
				...
				const Compositor_Props::Sequence_List &sequence= props->GetCompositorProps().Sequence.list;
				...
				if (sequence.size())
				{
					const Compositor_Props &props=m_CompositorProperties.GetCompositorProps();
					Compositor_Props &props_rw=m_CompositorProperties.GetCompositorProps_rw();
					size_t SequenceIndex=props.Sequence.CurrentSelection;
					m_Xpos=props.Sequence.list[SequenceIndex].PositionX;
					m_Ypos=props.Sequence.list[SequenceIndex].PositionY;
					UpdateSequence(props_rw.Sequence,SequenceIndex,true);
				}
		...
		Bitmap_Frame *Render_Reticle(Bitmap_Frame *Frame,const Compositor_Props::Sequence_List &sequence,size_t SequenceIndex)
		{
			...
			case Compositor_Props::eComposite:
				{
					const Compositor_Props::Sequence_List &composite=seq_pkt.specific_data.Composite->list;
		...
		Bitmap_Frame *TimeChange(Bitmap_Frame *Frame)
		{
			...
			if (SmartDashboard::IsConnected())
			{
				...
				UpdateSequence(props_rw.Sequence,(size_t)SmartDashboard::GetNumber("Sequence")-1); //convert to ordinal
			}
			return Render_Reticle(Frame,props.Sequence.list,props.Sequence.CurrentSelection);
		}

		Plugin_Controller_Interface* GetBypassPluginInterface(void) {return m_Bypass.GetPluginInterface();}
		Compositor_Props::ReticleType GetCurrentReticalType() const 
		{	return m_CompositorProperties.GetCompositorProps().Sequence.list[m_CompositorProperties.GetCompositorProps().Sequence.CurrentSelection].type;
		}
	private:
