...
void DDR_ViewMode::PaintWindow(HWND hWnd,HDC hdc,StoryBoard2_Crouton *sc) {
//	InOut_Wrapper *wrapper=&(s_CroutonListener.m_InOutWrapper);
	...
	if (sc->m_contentInstance) {
		...
		if (sc->m_IsAltKeyPressed) {
			//We can use any of the TimeCodeFloats to figure the height (but not necessarily the width)
			long TimeCodeYHeight=m_TimeCodeIn.ScreenObject_GetPreferedYSize();
			long WindowHeight=sc->GetWindowHeight();
			long IconWidth=WindowHeight*4/3;
			long NameWidth=m_textItemPainted.ScreenObject_GetPreferedXSize();
			//calculate the top offset   parent-child/2
			long YCenterOffset=(WindowHeight-TimeCodeYHeight)>>1;

			rect.left+=IconWidth+NameWidth+20;
			rect.right-=4;
			rect.top=YCenterOffset;
			rect.bottom=rect.top+TimeCodeYHeight;
			...
			if (l_SubRegionFocus==1) {
				...
				rect.right=rect.left+m_TimeCodeOut.ScreenObject_GetPreferedXSize();
...
ScreenObject* DDR_ViewMode::GetFileIcon(StoryBoard2_Crouton *sc) {
...
void DDR_ViewMode::MouseLButtonClick(long Flags,long x,long y,StoryBoard2_Crouton *sc) {
...
void DDR_ViewMode::MouseLButtonRelease(long Flags,long x,long y,StoryBoard2_Crouton *sc) {
	if (sc->Clicked) {
...
void DDR_ViewMode::MouseMoved(long Flags,long x,long y,StoryBoard2_Crouton *sc) {
...
void DDR_ViewMode::MouseEnter(StoryBoard2_Crouton *sc) {
	sc->StartTimer((unsigned)sc,100); sc->OnTimer((unsigned)sc);
	sc->VideoEditor_Crouton::MouseEnter();
	}

void DDR_ViewMode::MouseExit(StoryBoard2_Crouton *sc)  {
...
void DDR_ViewMode::OnTimer(unsigned long TimerID,StoryBoard2_Crouton *sc) {
...
unsigned DDR_ViewMode::GetSubRegionFocus(StoryBoard2_Crouton *sc) {
...
char *DDR_ViewMode::FindAStream(InOut_Wrapper *wrapper) {
	char *A_Stream=NULL;
...
double DDR_ViewMode::SetInPoint(InOut_Wrapper *wrapper,double adjustment,char *stream) {
	if (wrapper) {
...
double DDR_ViewMode::GetInPoint(InOut_Wrapper *wrapper,char *stream) {
	if (wrapper) {
		if (!strcmp(stream,InOuts2_All)) {
...
double DDR_ViewMode::SetOutPoint(InOut_Wrapper *wrapper,double adjustment,char *stream) {
	if (wrapper) {
		if (!strcmp(stream,InOuts2_All)) {
			//set the points of all the streams
			tList<char *> streamlist;wrapper->InOut_GetStreamTypes(&streamlist);
...
double DDR_ViewMode::GetOutPoint(InOut_Wrapper *wrapper,char *stream) {
	if (wrapper) {
		if (!strcmp(stream,InOuts2_All)) {
