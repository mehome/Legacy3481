...
TimeLine_Editor::TimeLine_Editor() {
	m_skin = NULL;
	m_Skin=TimeLine_Editor_SkinRoot;
...
void TimeLine_Editor::InitialiseWindow(void) {
	// Set up skin window
...
void TimeLine_Editor::DestroyWindow(void) {	
...
VideoEditor_Interface* TimeLine_Editor::CreateVideoEditor(unsigned color)
{
	...
	if (timeLine)
	{
		timeLine->VideoEditor_SetTimeLine(m_timeLine);
...
void TimeLine_Editor::VideoEditorClient_SetTimeLine(TimeLine *timeline) {
	m_timeLine=timeline;
	if (m_layout) m_layout->VideoEditor_SetTimeLine(m_timeLine);
	}

void TimeLine_Editor::InitializeSubControls()
...
bool TimeLine_Editor::ExecuteCommand(	char *FunctionName,						// The string representation of the command to execute
