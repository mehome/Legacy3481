...
HANDLE arrayofevents[EVENT_MAX];
HWND About;
...
LRESULT CALLBACK handlemain(HWND w_ptr, UINT uMsg, WPARAM wParam, LPARAM lParam) {
	...
	switch (uMsg) {
		case WM_APP: {
			struct avihandle *avivar=(struct avihandle *)lParam;
			if (wParam==100) {
				return (AVIFileOpen(&(avivar->avifile),avivar->filesource,OF_READ,NULL));
				}
			break;
			}
		case WM_CREATE:
			memset(arrayofevents,0,EVENT_MAX*4);
			m_ptr=GetMenu(w_ptr);
		...
		case WM_SYSCOMMAND:
			...
			if ((wParam==SC_RESTORE)&&(!preview->previewoff)) preview->showoverlay();
...
void cleanup() {
	...
	killdve1thread=
	killdve2thread=TRUE;
	for (t=0;t<EVENT_MAX;t++) if(arrayofevents[t]) SetEvent(arrayofevents[t]);
	...
	for (t=0;t<EVENT_MAX;t++) CloseHandle(arrayofevents[t]);
...
char *getdirectory(char *inputprompt,char *pathholder) {
	...
	if (result) SHGetPathFromIDList(result,pathholder);
