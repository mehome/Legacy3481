...
int FF_Play_Reader::get_video_frame(AVFrame *frame, int64_t *pts, AVPacket *pkt, int *serial)
{
    ...
    if (got_picture) 
	{
		if (m_realtime)
		{
			while (m_FrameConsumedCounter>4)
			{
				AVPacket null_pkt;
				...
				null_pkt.data = NULL;
				null_pkt.size = 0;
				if(avcodec_decode_video2(m_video_st->codec, frame, &got_picture, &null_pkt) < 0)
					break;
				m_FrameConsumedCounter--;
				FrameWork::DebugOutput("reduce latency count=%d\n",m_FrameConsumedCounter);
			}
		}
        int ret = 1;
