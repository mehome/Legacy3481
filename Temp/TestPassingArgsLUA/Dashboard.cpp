...
public:
	...
	DDraw_Preview_Properties(const char *FileName=NULL,const char *Arg=NULL) : m_UsingLUA(false),m_SaveOnExit(false)
	{
		DDraw_Preview::DDraw_Preview_Props &props=m_PreviewProps;
		...
		if ((ext)&&(stricmp(ext,".lua")==0))
		{
			...
			if (err==NULL)
			{
				...
				{
					char *Buffer;
					//err = script.CallFunction("SetupConfiguration", "s>", Arg);
					lua_pushstring(script.Get_LUA_State(),"black");
					lua_setglobal(script.Get_LUA_State(),"URL_ToUse");
					err = script.CallFunction("Test", ">s", &Buffer);
					int x=8;
				}
					   ...
					   int       nCmdShow)
{
	...
	{
		wchar2char(lpCmdLine);
		split_arguments(wchar2char_pchar,Args);
	}
...

	{
		...
		#endif
		DDraw_Preview_Properties main_props(Filename.c_str()[0]!=0?Filename.c_str():NULL,Args.size()>1?Args[1].c_str():NULL);
